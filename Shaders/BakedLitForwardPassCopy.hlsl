...
struct Varyings
{
    ...
    UNITY_DEFINE_INSTANCED_PROP(float4, custom_lightST)
...
Varyings BakedLitForwardPassVertex(Attributes input)
{
    ...
    float4 l = UNITY_ACCESS_INSTANCED_PROP(Props, custom_lightST);
    
    if (l.x == 0)
        l = unity_LightmapST;
    
    
    OUTPUT_LIGHTMAP_UV(input.staticLightmapUV, l, output.staticLightmapUV);
